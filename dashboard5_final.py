...
def get_ticker_data():
    ...
    stocks = ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "TESLA", "JNJ"]
    data = []
    for s in stocks:
        try:
            t = yf.Ticker(s)
            ...
            if not h.empty:
                p = h['Close'].iloc[-1]
                c = ((p - h['Close'].iloc[0]) / h['Close'].iloc[0] * 100) if len(h) > 1 else 0
                data.append({"sym": s, "price": f"${p:.2f}", "chg": f"{c:+.1f}%", "col": "#00ff88" if c >= 0 else "#ff4757"})
        except: pass
    return data
...
html = '<style>'
html += '.ticker {background: #0a0a0f; border-bottom: 2px solid #00ff88; padding: 15px; margin-bottom: 2rem; overflow: hidden;}'
html += '.ticker-head {font-size: 2rem; font-weight: 900; color: #00ff88; margin-bottom: 10px; font-family: Syne;}'
html += '.ticker-sub {font-size: 0.7rem; color: #64748b; letter-spacing: 0.1em; text-transform: uppercase;}'
html += '.ticker-scroll {display: flex; gap: 20px; animation: scroll 30s linear infinite; white-space: nowrap;}'
html += '@keyframes scroll {0% {transform: translateX(0);} 100% {transform: translateX(-50%);}}'
html += '.ticker-item {display: inline-flex; gap: 10px; padding: 0 15px; align-items: center; min-width: 150px;}'
html += '.ticker-item span {font-family: JetBrains Mono; font-weight: 700;}'
html += '@media (max-width: 768px) {.ticker-head {font-size: 1.3rem;} .ticker-scroll {animation: scroll 20s linear infinite;}}'
html += '</style>'

html += '<div class="ticker"><div class="ticker-head">EDGAR</div><div class="ticker-sub">by Enrique C. · Investment Intelligence V3</div>'
html += '<div class="ticker-scroll">'

for item in ticker_items + ticker_items:
    html += f'<div class="ticker-item"><span>{item["sym"]}</span><span>{item["price"]}</span><span style="color:{item["col"]}">{item["chg"]}</span></div>'
